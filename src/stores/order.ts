...
export const useOrderStore = defineStore('order', () => {
  ...
  const cartItemCount = computed(() => {
    const cartCount = cart.value.cartItems.reduce((accumulator, cartItem) => accumulator + cartItem.quantity, 0);
    return cartCount;
  });
  const cartTotal = computed(() => {
    return cart.value.cartItems.reduce((accumulator, cartItem) => accumulator + cartItem.price * cartItem.quantity, 0);
  });
  ...
  async function applyDiscounts() {
    ...
    if (multiPlantDiscount) {
      if (cartItemCount.value >= multiPlantDiscount.parameters.minimumQuantity) {
        discounts.discountValues.reduce(function (acc, obj) {
  ...
  async function getActiveDiscounts() {
    ...
    if (!discountDocs.value || discountDocs.value.length === 0) {
      return undefined;
    }
