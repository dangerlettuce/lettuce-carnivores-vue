...
export const useGiveawayStore = defineStore('giveaway', () => {
  ...
  function addLetter() {
    if (remainingLetters.value.length === 0 || myBlanks.value.length === 0) {
      console.error('no remaining letters / blanks');
      return;
    }
    const newLetter = remainingLetters.value.shift();
    myLetters.value.push(newLetter!);
    myBlanks.value.pop();
    if (myBlanks.value.length === 0 || myLetters.value.length >= password.value.length) {
      checkAnswer();
    }
    return newLetter;
  }
  ...
  async function submitGiveawayEntry(entryData: GiveawayFormData) {
    ...
    const giveawayEntry: GiveawayEntry = {
      giveawayName: giveawayName.value,
      ...entryData,
      uid: userStore.user?.uid ?? '',
      entryStartTime: entryStartTime.value,
      entryEndTime: entryEndTime.value,
      bypassGame: bypassGame.value,
    };
  ...
  async function fetchActiveGiveaway() {
    ...
    if (giveawayDetails.value !== undefined) {
      return;
    }
