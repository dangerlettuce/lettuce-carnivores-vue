          ...
          <div class="shipping-info my-3">
            ...
            <div class="flex justify-around gap-2">
              ...
              <div>
                ...
                <div>
                  {{ order.orderStatus.trackingNumber !== '' ? order.orderStatus.trackingNumber : 'Tracking number not yet assigned' }}
                </div>
              </div>
              <div v-if="isAdmin" class="admin-actions">
                <BaseButton @click="openOrderStatusModal(order)" :key="order.id" type="info">Update Status</BaseButton>
                <BaseButton v-if="order.orderStatus.status === 'Processing'" @click="sendOrderShippedEmail(order)"
                  >Send Shipped Notification</BaseButton
                >
                <BaseButton v-if="order.orderStatus.status === 'Shipped'" @click="markOrderComplete(order)" :key="order.id"
                  >Mark Complete</BaseButton
                >
              </div>
            </div>
          </div>
