        ...
        <Accordion type="single" class="w-full" collapsible :default-value="'0'">
            <AccordionItem v-for="order of props.orders" :key="order.id" :value="order.id.toString()">
                <AccordionTrigger class="accordion-header"
                    :pt="{ toggleicon: { style: { 'margin-left': 'auto', 'margin-right': '.6rem' } } }">
                    ...
                    <div class="shipping-info my-3">
                        ...
                        <div class="flex justify-around gap-2">
                            <div class="ms-3">
                                ...
                                <div v-if="order.shippingInfo.address.line2">{{ order.shippingInfo.address.line2 }}
                                </div>
                                <div>
                            ...
                            <div v-if="isAdmin" class="admin-actions">
                                <BaseButton @click="openOrderStatusModal(order)" :key="order.id" type="info">Update
                                    Status</BaseButton>
                                <BaseButton v-if="order.orderStatus.status === 'Processing'"
                                    @click="sendOrderShippedEmail(order)">Send Shipped Notification</BaseButton>
                                <BaseButton v-if="order.orderStatus.status === 'Shipped'"
                                    @click="markOrderComplete(order)" :key="order.id">Mark Complete</BaseButton>
                            </div>
                        </div>
                    </div>
                    <hr />
