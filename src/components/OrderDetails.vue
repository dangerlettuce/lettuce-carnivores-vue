      ...
      <table class="item-detail-table">
        ...
        <tbody>
          <tr v-for="item in order.lineItems" :key="item.price_data.product_data.metadata.sku">
            ...
            <td data-th="Shelf" v-if="isAdmin" class="justify-center">
              {{ item.price_data.product_data.metadata?.shelfLocation ? item.price_data.product_data.metadata?.shelfLocation : '' }}
            </td>
        ...
        <tfoot>
          <tr v-if="hasDiscount" class="top-border">
            ...
            <td data-th="Discount" class="justify-right">
              {{ `${hasDiscount ? '-' + USDollar.format(order.cartTotal.amount_discount / 100) : ''}  ` }}
            </td>
          </tr>
          <tr>
