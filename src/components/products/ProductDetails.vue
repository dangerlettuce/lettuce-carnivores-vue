    ...
    <article class="product-information">
      ...
      <div class="flex justify-evenly">
        <BaseButton
          ...
          {{ `${plant.sku}${plant.size !== 'each' ? ` - ${plant.size}` : ''} ` }}
        </BaseButton>
      </div>
...
<script setup lang="ts">
  ...
  onMounted(async () => {
    handleShowHidden(route);
    await fetchData();
    plants.value = getAvailablePlants(plantCategory.value, showHidden.value, hiddenPassword.value);
  ...
  const hiddenPassword = ref<string | null>(null);
  function handleShowHidden(route: RouteLocationNormalized) {
    if ('showHidden' in route.query || 'showhidden' in route.query || 'hidden' in route.params || route.path.includes('/hidden')) {
      showHidden.value = true;
    }
    if ('password' in route.params) {
      const password = Array.isArray(route.params.password) ? route.params.password[0] : route.params.password;
  ...
  const plantTypeLabel = computed(() => {
    ...
    return selectedPlant.value.isRepresentative ? 'Representative' : 'Specimen';
  });
  ...
  const plantTypeDescription = computed(() => {
    ...
    return selectedPlant.value.isRepresentative
      ? 'The photos show a plant or item that is similar to what you will receive.'
      : 'The photos show the exact plant/item your will receive.';
  });
