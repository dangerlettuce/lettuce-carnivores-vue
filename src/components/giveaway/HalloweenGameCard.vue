      ...
      <div v-show="showGhost" @click="showGhost = !showGhost">
        <div class="ghost-card">
          ...
          <div class="message-container">
            <div class="ghost-message">{{ ghostMessage }}</div>
            <h4></h4>
          </div>
        </div>
      </div>
...
function trick() {
  ...
  if (randomTrick < 0.25) {
    blackout();
  } else if (randomTrick < 0.5) {
    upsideDownScreen();
  } else if (randomTrick < 0.75) {
...
function appendClassToBody(className: string, timeout = 2000) {
  ...
  setTimeout(() => {
    bodyEle.classList.remove(className);
  }, timeout);
}
...
.flashlight {
  ...
  left: 0;
  top: 0;
  background: radial-gradient(circle at var(--cursor-x) var(--cursor-y), transparent 25px, rgba(0, 0, 0, 0.94) 150px);
}
