...
export default onCall({}, async (request: GiveawayEntry): Promise<FunctionResponse> => {
  ...
  if (!uid || !request.auth || !isDataValid || !isUniqueEntry) {
    ...
    if (!isDataValid) {
      return {
        success: false,
        error: true,
        message: `Error submitting giveaway entry\nEmail may be invalid`,
        errorDetails: { isDataValid: isDataValid, isUniqueEntry: isUniqueEntry },
        data: request.data,
      };
    }
    if (!isUniqueEntry) {
  ...
  try {
    await admin
      ...
      .doc(request.data.id.toString())
      .set({ ...request.data });
    return {
      success: true,
      error: false,
      message: `Success! Entry ID ${request.data.giveawayName}-${request.data.id}`,
      errorDetails: null,
      data: null,
    };
  } catch (e: any) {
    ...
    const query = admin
      ...
      .where(
        Filter.and(
          ...
          Filter.or(Filter.where('uid', '==', request.auth.uid), Filter.where('email', '==', request.data.email)),
        ),
      );
