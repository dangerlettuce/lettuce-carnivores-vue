...
interface EmailRequest extends CallableRequest {
  data: Order;
}

export const sendOrderStatusEmail = onCall(
  { secrets: ['EMAIL_USER', 'EMAIL_PASS'] },
  async (request: EmailRequest): Promise<FunctionResponse> => {
    ...
    if (!html) {
      return {
        ...
        errorDetails: 'Unable to generate email template',
        data: html,
      };
    }
...
async function sendMailAndSaveToDb(emailConfig: SMTPTransport.Options, id: string | number) {
  ...
  try {
    await admin
      ...
      .doc(id.toString())
      .set({ ...emailConfig, sentAt: Timestamp.now() });
