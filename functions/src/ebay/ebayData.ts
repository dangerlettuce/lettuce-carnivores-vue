...
export async function createOrReplaceInventoryItem(token: string, sku: string, item: any, plantCategoryId: string) {
  ...
  if (res && 'status' in res && res.status === 204) {
    admin
      ...
      .doc(sku)
      .set({ ...getUpdateDateTime(), plantCategoryId, sku }, { merge: true });
    return { success: true };
  }
...
export async function postEbayOffer(token: string, data: any, environment?: EbayEnvironment, offerId?: string) {
  ...
  if ((res && 'status' in res && res.status === 201) || res.status === 200 || res.status === 204) {
    admin
      ...
      .collection('inventory')
      .doc(data.sku)
      .set({ offerId: res.data.offerId, ...getUpdateDateTime() }, { merge: true });
...
export async function deleteEbayInventoryItem(sku: string) {
  ...
  const res = await axios.delete(url, config).catch((e) => {
    error(e);
  });
  if (!res || !res?.status) {
...
export async function getListingsData(
  ...
  token: string,
  granularityLevel: GranularityLevel = 'Medium',
  daysAgo: number = 120,
) {
  if (daysAgo > 121) {
    return { error: true, success: false, message: 'Cannot query more than 121 days' };
  }
  ...
  const res = await axios.post(`${baseUrl}/ws/api.dll`, xmlBody, config).catch((e) => {
    return e;
  });
  if (res.status === 200) {
